            ...
            <div class="span12">
              ...
              <div class="widget">
                <div class="widget-body">
                  <!-- BEGIN FORM-->
                  <form action="<?php echo (C("ACTION_BASE_DIR")); ?>/employee/employeeUpdate/" class="form-horizontal" id="employee_form">
                    <div class="control-group">
                      <label class="control-label">员工姓名:</label>
                      <div class="controls">
                        <input type="text" placeholder="员工姓名" class="span5" name="employee_name" value="<?php echo ($data["employee_name"]); ?>" />
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label">员工性别</label>
                      <?php $employee_sex = $data["employee_sex"]; ?>
                      <div class="controls">
                        <label class="radio">
                            <input type="radio" name="employee_sex" value="0" <?php echo ($employee_sex == 0 ? "checked":""); ?>/>男
                        </label>
                        <label class="radio">
                            <input type="radio" name="employee_sex" value="1" <?php echo ($employee_sex == 1 ? "checked":""); ?>/>女
                        </label>
                      </div>
                    </div>
                    <div class="control-group">
                        ...
                        <select data-placeholder="请选择级别" class="span5" tabindex="-1" id="employee_position_id">
                          ...
                          <?php if(is_array($employeePositionBigList)): $i = 0; $__LIST__ = $employeePositionBigList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$employee_postion_big): $mod = ($i % 2 );++$i;?><optgroup label="<?php echo ($employee_postion_big["employee_position_big_name"]); ?>">
                              ...
                              <?php if(is_array($employeePositionList)): $i = 0; $__LIST__ = $employeePositionList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$employee_postion): $mod = ($i % 2 );++$i; $employee_postion_sub_id = $employee_postion["employee_position_big_id"]; ?>
                                <?php if($employee_postion_big_id == $employee_postion_sub_id): $select_value = $employee_postion["employee_position_id"]; ?>
                                  <?php $selected_value = $data["employee_position_id"]; ?>
                                  <option value="<?php echo ($employee_postion["employee_position_id"]); ?>" <?php echo ($select_value == $selected_value ? "selected":""); ?>><?php echo ($employee_postion["employee_position_name"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                            </optgroup><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                        <input type="hidden" name="employee_position_id" value="<?php echo ($data["employee_position_id"]); ?>"/>
                      </div>
                    </div>
                    <div class="control-group">
                      ...
                      <div class="controls">
                        <label class="radio">
                            <input type="radio" name="employee_status" value="0" <?php echo ($employee_status == 0 ? "checked":""); ?>/>在职
                        </label>
                        <label class="radio">
                            <input type="radio" name="employee_status" value="1" <?php echo ($employee_status == 1 ? "checked":""); ?>/>离职
                        </label>
                      </div>
                    </div>
                    <div class="control-group">
                      ...
                      <div class="controls">
                        <input type="text" placeholder="员工电话" class="span5" name="employee_tel" value="<?php echo ($data["employee_tel"]); ?>"/>
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label">员工身份证号码:</label>
                      <div class="controls">
                        <input type="text" placeholder="员工身份证号码" class="span5" name="employee_id_card" value="<?php echo ($data["employee_id_card"]); ?>"/>
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label">员工银行账户:</label>
                      <div class="controls">
                        <input type="text" placeholder="员工银行账户" class="span5" name="employee_bank_no" value="<?php echo ($data["employee_bank_no"]); ?>"/>
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label">员工出生日期</label>
                      <div class="controls">
                        <input size="16" type="text" class="datepicker_field m-ctrl-medium" readonly name="employee_birthday"  value="<?php echo ($data["employee_birthday"]); ?>"/>
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label">员工入职日期</label>
                      <div class="controls">
                      	<input size="16" type="text" class="datepicker_field m-ctrl-medium" readonly name="employee_work_date" value="<?php echo ($data["employee_work_date"]); ?>"/>
                      </div>
                    </div>
                    <div class="control-group">
          						<label class="control-label" >备注:</label>
          						<div class="controls" >
          							<textarea class="span5 " rows="3" name="employee_comment"><?php echo ($data["employee_comment"]); ?></textarea>
          						</div>
          					</div>
                    <input type="hidden" placeholder="" class="span12" name="employee_id" value="<?php echo ($data["employee_id"]); ?>"/>
...
<div class="body_overlay">
  <table>
    ...
    <script type='text/javascript' src="<?php echo (C("WEB_RES_ROOT")); ?>/jquery.ui.datepicker.js"></script>
