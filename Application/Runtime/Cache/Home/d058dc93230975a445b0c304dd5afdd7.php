              ...
              <div class="widget">
                <div class="widget-body">
                  <table class="table table-bordered table-hover table-header">
                    <thead>
                      <tr>
                        <th class="span1 cell_middle" rowspan="2">编号</th>
                        <th class="span2 cell_middle" rowspan="2">名称</th>
                        <th class="span1 cell_middle" rowspan="2">类型</th>
                        <th class="span6" colspan="6">详细内容</th>
                        <th class="span2 cell_middle" rowspan="2">操作</th>
                      </tr>
                      <tr>
                        <th class="span1">编号</th>
                        <th class="span1">项目名称</th>
                        <th class="span1">次数</th>
                        <th class="span1">单价</th>
                        <th class="span1">总价</th>
                        <th class="span1">有效期</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php if(is_array($courseList)): $i = 0; $__LIST__ = $courseList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$course): $mod = ($i % 2 );++$i;?><tr>
                          <td class="cell_middle"><?php echo ($course["course_id"]); ?></td>
                          <td class="cell_middle"><?php echo ($course["course_name"]); ?></td>
                          <td class="cell_middle"><?php echo ($course["category_name"]); ?></td>
                          <td class="nopadding" colspan="6">
                            <table class="table table_inner">
                      				<?php if(is_array($courseServiceList)): $i = 0; $__LIST__ = $courseServiceList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$course_service_list): $mod = ($i % 2 );++$i; if($course["course_id"] == $course_service_list['course_id']): ?><tr>
                        						<td class="span1"><?php echo ($course_service_list["service_id"]); ?></td>
                        						<td class="span1"><?php echo ($course_service_list["service_name"]); ?></td>
                        						<td class="span1"><?php echo ($course_service_list["usecount"]); ?></td>
                        						<td class="span1"><?php echo ($course_service_list["signal_price"]); ?></td>
                        						<td class="span1"><?php echo ($course_service_list["course_service_price"]); ?></td>
                        						<td class="span1">
                        							<?php if($course_service_list['course_service_type'] == 0): echo ($course_service_list["course_service_days"]); ?>天
                        							<?php else: ?>
                            ...
                            <button class="btn btn-danger" onClick="ajaxRequest('/course/courseDelete/','/course/courseList/','{course_id:<?php echo ($course["course_id"]); ?>,course_name:\'<?php echo ($course["course_name"]); ?>\'}','确定删除？')">删除</button>
                          </td>
                				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    </tbody>
                  </table>
                  <button class="btn btn-primary" onClick = "toLink('/course/toCourseAdd/','')">添加套餐</button>
                  <button class="btn btn-primary" onClick = "toLink('/course/toCourseCategoryProcess/','')">添加套餐类型</button>
                </div>
              </div>
            </div>
          </div>
