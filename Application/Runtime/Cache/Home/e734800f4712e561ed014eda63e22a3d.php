            ...
            <div class="span12">
              <div class="widget">
                <div class="widget-body">
                  <?php if($adminCount != 0 ): ?><table class="table table-bordered table-hover table-header">
                      <thead>
                        <tr>
                          <th class="span2">管理员用户名</th>
                          <th class="span2">管理员邮箱</th>
                          <th class="span2">管理员姓名</th>
                          <th class="span2">管理员电话</th>
                          <th class="span3"></th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php if(is_array($adminlist)): $i = 0; $__LIST__ = $adminlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$admin): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                            <td><?php echo ($admin["admin_name"]); ?></td>
                            <td><?php echo ($admin["admin_mail"]); ?></td>
                            <td><?php echo ($admin["admin_real_name"]); ?></td>
                            <td><?php echo ($admin["admin_tel"]); ?></td>
                            <td>
                              <button class="btn btn-primary" onClick="toLink('/admin/toAdminUpdate/','{admin_id:<?php echo ($admin["admin_id"]); ?>,upd_kbn:0}')">修改基本信息</button>
                              <button class="btn btn-primary" onClick="toLink('/admin/toAdminUpdate/','{admin_id:<?php echo ($admin["admin_id"]); ?>,upd_kbn:1}')">修改密码</button>
                              <button class="btn btn-primary" onClick="toLink('/admin/toAdminUpdate/','{admin_id:<?php echo ($admin["admin_id"]); ?>,upd_kbn:2}')">分配权限</button>
                              <button class="btn btn-danger" onClick="ajaxRequest('/admin/adminDelete/','/admin/adminList/','{admin_id:<?php echo ($admin["admin_id"]); ?>,admin_name:\'<?php echo ($admin["admin_name"]); ?>\'}','确定删除？')">删除</button>
                            </td>
                  ...
                  <button class="btn btn-primary" onClick = "toLink('/admin/toAdminAdd/','')">添加管理员</button>
                </div>
              </div>
            </div>
          </div>
