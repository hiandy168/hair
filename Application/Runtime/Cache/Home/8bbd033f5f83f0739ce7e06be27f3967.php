              ...
              <div class="widget">
                <div class="widget-body">
                  <table class="table table-striped table-bordered table-hover" id="data">
                    ...
                    <tbody>
                      <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$employee): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
                          <td><?php echo ($employee["employee_no"]); ?></td>
                          <td><?php echo ($employee["employee_name"]); ?></td>
                          <td><?php echo ($employee["employee_position_name"]); ?></td>
                          <td>
                            <?php $employee_sex = $employee["employee_sex"]; ?>
                            <?php if($employee_sex == '0'): ?>男 <?php else: ?> 女<?php endif; ?>
                          </td>
                          <td><?php echo ($employee["employee_tel"]); ?></td>
                          <td><?php echo ($employee["employee_work_date"]); ?></td>
                          <td>
                            <?php $employee_status = $employee["employee_status"]; ?>
                            <?php if($employee_status == '0'): ?>在职 <?php else: ?> 离职<?php endif; ?>
                          </td>
                          <td>
                            <button class="btn btn-primary" onClick="employee_update('<?php echo ($employee["employee_id"]); ?>')">修改</button>
                            <button class="btn btn-danger" onClick="employee_delete('<?php echo ($employee["employee_id"]); ?>')">删除</i></button>
                          </td>
